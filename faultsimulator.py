#!/usr/bin/env python3

from random import randrange
import argparse, sys, os

sbox = []

isbox = []

rcon = [0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36]

def xtime(a):
    return ((a << 1) & 0xff) ^ (a >> 7)*0x1b

def mixcolumn(b):
    t = b[0] ^ b[1] ^ b[2] ^ b[3]
    u = b[0]

    v = b[0] ^ b[1]
    v = xtime(v)
    b[0] = b[0] ^ v ^ t

    v = b[1] ^ b[2]
    v = xtime(v)
    b[1] = b[1] ^ v ^ t

    v = b[2] ^ b[3]
    v = xtime(v)
    b[2] = b[2] ^ v ^ t

    v = b[3] ^ u
    v = xtime(v)
    b[3] = b[3] ^ v ^ t


def imixcolumn(b):
    # preprocessing
    u = b[0] ^ b[2]
    u = xtime(xtime(u))
    v = b[1] ^ b[3]
    v = xtime(xtime(v))
    b[0] ^= u
    b[1] ^= v
    b[2] ^= u
    b[3] ^= v

    # mixcolumn
    mixcolumn(b)


def MC(state):
    for col in state:
        mixcolumn(col)


def iMC(state):
    for col in state:
        imixcolumn(col)


def SR(state):
    state[0][1], state[1][1], state[2][1], state[3][1] = state[1][1], state[2][1], state[3][1], state[0][1]
    state[0][2], state[1][2], state[2][2], state[3][2] = state[2][2], state[3][2], state[0][2], state[1][2]
    state[0][3], state[1][3], state[2][3], state[3][3] = state[3][3], state[0][3], state[1][3], state[2][3]


def iSR(state):
    state[0][1], state[1][1], state[2][1], state[3][1] = state[3][1], state[0][1], state[1][1], state[2][1]
    state[0][2], state[1][2], state[2][2], state[3][2] = state[2][2], state[3][2], state[0][2], state[1][2]
    state[0][3], state[1][3], state[2][3], state[3][3] = state[1][3], state[2][3], state[3][3], state[0][3]


def SB(state):
    for i in range(4):
        state[i] = [sbox[b] for b in state[i]]


def iSB(state):
    for i in range(4):
        state[i] = [isbox[b] for b in state[i]]


def AK(state, roundkeys, rnd):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= roundkeys[rnd*4 + i][j]


def key_expansion(masterkey):
    roundkeys = [masterkey[i:i+4] for i in range(0, 16, 4)]
    for rnd in range(10):
        for j in range(4):
            tmp = roundkeys[-1]
            if j == 0:
                tmp = tmp[1:] + tmp[:1]
                tmp = [sbox[t] for t in tmp]
                tmp[0] ^= rcon[rnd]
            rk = [tmp[i] ^ roundkeys[rnd*4 + j][i] for i in range(4)]
            roundkeys.append(bytes(rk))

    return roundkeys


def encrypt_aes(plaintext, masterkey, **kwargs):
    fault_round = kwargs.get('fault_round')
    fault_pos = kwargs.get('fault_pos')
    fault = kwargs.get('fault')
    dump_rk = kwargs.get('dump_rk', False)

    roundkeys = key_expansion(masterkey)
    if dump_rk:
        print('roundkeys')
        for i in range(0, len(roundkeys), 4):
            print(f'{i:2}', b''.join(roundkeys[i:i+4]).hex())

    state = [bytearray(plaintext[i:i+4]) for i in range(0,16,4)]
    AK(state, roundkeys, 0)

    # round 1 to nr-1
    for rnd in range(1, 10):
        SB(state)
        SR(state)
        if rnd == fault_round:
            state[fault_pos // 4][fault_pos % 4] ^= fault

        MC(state)
        AK(state, roundkeys, rnd)

    # last round
    SB(state)
    SR(state)
    AK(state, roundkeys, 10)

    return bytes([state[i][j] for i in range(4) for j in range(4)])


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='AES fault simulator')
    parser.add_argument('-r', action='store', dest='rnd', type=int,
                        help='Round where the fault is made (8 or 9)',
                        required=True)
    parser.add_argument('-n', action='store', dest='n', type=int,
                        default=1, help='Number of sets of four correct/faulty ciphertexts in "-r 9" mode, and number of correct/faulty ciphertexts in "-r 8" mode')
    parser.add_argument('--keeppos', action='store_true', dest='keeppos',
                        default=False, help='Store fault positions')
    parser.add_argument('--keepfault', action='store_true', dest='keepfault',
                        default=False, help='Store fault positions and values')
    parser.add_argument('--bitflip', action='store_true', dest='bitflip',
                        default=False, help='Fault values are bitflips')
    args = parser.parse_args()

    if args.rnd not in [8, 9]:
        sys.exit()

    if args.n < 1:
        sys.exit()

    if args.keepfault:
        args.keeppos = True

    # generate master key and pair of correct plaintext/ciphertext
    zero_block=b'\0'*16
    masterkey = zero_block#os.urandom(16)
    pt0 = zero_block#os.urandom(16)
    ct0 = bytes(encrypt_aes(pt0, masterkey, dump_rk=False))

    print(f'{pt0.hex()},{ct0.hex()}')

    # generate pairs of correct/faulty ciphertexts
    if args.rnd == 8:
        for i in range(args.n):
            pt = os.urandom(16)
            fault_pos = randrange(0, 16)
            if args.bitflip:
                fault = 1 << randrange(0, 8)
            else:
                fault = randrange(1, 256)

            ct = bytes(encrypt_aes(pt, masterkey))
            fct = bytes(encrypt_aes(pt, masterkey, fault_round=8, fault_pos=fault_pos, fault=fault))

            s = f'{ct.hex()},{fct.hex()}'
            if args.keeppos:
                s += f',{fault_pos}'
            if args.keepfault:
                s += f',{fault}'
            print(s)
    else:
        for i in range(args.n):
            # four pairs with a fault in a different column
            for j in range(4):
                fault_pos = randrange(4*j, 4*(j + 1))
                if args.bitflip:
                    fault = 1 << randrange(0, 8)
                else:
                    fault = randrange(1, 256)

                pt = os.urandom(16)
                ct = bytes(encrypt_aes(pt, masterkey))
                fct = bytes(encrypt_aes(pt, masterkey, fault_round=9, fault_pos=fault_pos, fault=fault))

                s = f'{ct.hex()},{fct.hex()}'
                if args.keeppos:
                    s += f',{fault_pos}'
                if args.keepfault:
                    s += f',{fault}'
                print(s)

